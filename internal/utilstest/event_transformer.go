...
func (r *Transformer) WithError(err error) *Transformer {
	r.MockTransformer.EXPECT().Transform(gomock.Any()).Return(nil, err)
...
func (r *Transformer) WithNoResults() *Transformer {
	r.MockTransformer.EXPECT().Transform(gomock.Any()).Return(nil, nil)
...
func (r *Transformer) WithResult(message *ingestor.BaseEvent) *Transformer {
	r.MockTransformer.EXPECT().Transform(gomock.Any()).Return(message, nil)
