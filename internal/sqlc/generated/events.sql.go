...
const summarizeByDomain = `-- name: SummarizeByDomain :many
...
type SummarizeByDomainRow struct {
	Domain    string
	EventType string
	Cnt       int64
	FirstSeen interface{}
...
func (q *Queries) SummarizeByDomain(ctx context.Context, occurredAt time.Time) ([]SummarizeByDomainRow, error) {
	rows, err := q.db.QueryContext(ctx, summarizeByDomain, occurredAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SummarizeByDomainRow
	for rows.Next() {
		var i SummarizeByDomainRow
		if err := rows.Scan(
			&i.Domain,
			&i.EventType,
			&i.Cnt,
